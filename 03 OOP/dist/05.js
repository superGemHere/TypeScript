class Employee {
    ...
    age;
    email;
    constructor(name, salary, position, department, age, email) {
        this.name = name;
        this.salary = salary;
...
class Department {
    ...
    age;
    email;
    constructor(name, salary, position, age, email) {
        this.department = {
            name: name,
            salary,
            position,
...
function highestPaidDepartment(arr) {
    let itterations = Number(arr[0]);
    let companyList = {};
    for (let i = 1; i <= itterations; i++) {
        let currentArr = arr[i].toString().split(' ');
        let name = currentArr[0];
        let salary = Number(currentArr[1]);
        let position = currentArr[2];
        let department = currentArr[3];
        let email = "n/a";
        let age = -1;
    ...
    let highestAverage = Object.entries(companiesAvg)[0];
